...
vi.mock("../../src/auth", () => ({ getAccessToken: vi.fn(async () => "token") }));
vi.mock("../../src/http-client", () => ({ default: { post: shared.post } }));
vi.mock("../../src/card-callback-service", () => ({ updateCardVariables: shared.update }));
vi.mock("../../src/inbound-handler", () => ({ handleDingTalkMessage: shared.inbound }));
