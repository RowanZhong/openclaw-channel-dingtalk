...
- **自定义插件** 负责个人偏好、监听、草稿、自动授权、通知和发送。两个仓库使用同一份 0.6.1 业务代码，社区适配层按两版 SDK 的导入路径分别编译。
...
操作卡片的截止时间由独立字段 `card_expires_note` 投递，模板在按钮下方以右对齐的小号备注显示“卡片有效期至 MM/DD HH:mm”。它只控制卡片回调；监听设置持续生效，草稿和自动答复授权分别检查自己的到期时间。从不含备注组件的早期版本升级时，需新建普通卡片模板、导入并发布配套 JSON，再更新 `assistant.cardTemplateId`。0.5.2 升级到本次 0.6.1 沿用已发布模板，无需再次发布；本次也未修改社区适配层。
...
需要同时安装对应的**改造版社区插件** 与 **dws-send-approval 0.6.1** 。不修改 OpenClaw 核心或 DWS 源码。上游 3.8.1 包仍要求新宿主，不可安装到旧宿主。

**从 0.6.0 升级到 0.6.1：** 本次只需替换自定义插件包并重启 Gateway；已安装的对应改造版社区插件、已发布模板及个人状态继续沿用。你们的 `main default:true + mail/come/bpm/code` 配置和现有 `agentId:"main"` 保持不变，不添加跨 Agent 授权。保留 PVC；修复后可对原失败记录点「重新拟稿」，或点「修改」手动答复。
...
```bash
# 旧宿主安装自定义插件
openclaw plugins install /absolute/path/dws-send-approval-0.6.1.tgz --force

# 新宿主安装自定义插件
openclaw plugins install /absolute/path/dws-send-approval-0.6.1.tgz --force --accept-capabilities
```
