...
- 之后按 `cardTaskProgressRefresh` 刷新阶段与耗时（默认每 30 秒）
...
`cardTaskProgressRefresh` 控制刷新节奏，即「阶段文案新鲜度」与「卡片更新调用量」之间的取舍：

| 值 | 说明 |
| --- | --- |
| `heartbeat`（默认） | 工具事件只更新阶段/步数状态，随下一次 30 秒刷新一起落盘；调用量固定为「出现时 1 次 + 每 30 秒 1 次」，工具密集的任务也不会增加；代价是阶段文案最多滞后 30 秒 |
| `interval` | 每次可关联的工具事件都推送阶段变化，由 `cardStreamInterval`（默认 `1000`ms）节流；阶段文案更及时，但调用量随工具调用次数增长，工具密集的长任务会明显更高 |
