...
- 等待时间仍为 5 分钟。截止时回传已收集结果，未回应的人标为 `missing`，整体标为 `expired`；旧卡不能继续提交。
- 群卡片仅显示收集进度，不展示填写内容；全部收齐前仍显示表单，已经填写的人再次提交会被忽略。定向卡片关闭转发，服务端仍校验实际回调账号和填写人，不能通过转发获得填写权限。
- 沿用原生命周期：发起人在同一 scope 发送新的普通消息、发起新的表单，或 gateway 重启，都会让旧表单失效。填写人在自己的其他会话发送消息不会替代发起人的表单。暂停原任务失败时卡片也会失效。
