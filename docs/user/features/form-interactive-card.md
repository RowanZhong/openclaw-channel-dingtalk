...
`dingtalk-form` Skill 将设计、查群找人、投放和回传连接起来。用户可以说“帮我收集项目交付风险”，由机器人拟定题目，通过卡片选择群、成员和时长，预览确认后再发送；已有完整内容时直接创建。指定填写人只需提供姓名，Skill 用 DWS 查询并解析表单要求的 `staffId`，遇到重名或查询不完整时先说明原因。

支持私聊或群聊发起、给本人或指定用户私发、向群内指定成员收集，结果固定回到原发起会话。重复工作可进一步配置[定时自动收集](scheduled-form-collection.md)。图解及可复制示例见[用户文档中的 HTML 使用指南](../index.md#互动表单图解)。

### 安装与组织绑定

Skill 作为仓库资源提供，需要单独安装到实际使用的 Agent 工作区；插件的 npm 包不会自动安装它。管理员需准备 `python3`、已登录的 DWS 1.0.58，以及支持本功能的插件。脚本仅依赖 Python 标准库。

在本仓库根目录执行以下命令，使用自定义工作区时将目标路径替换为实际位置：

```sh
mkdir -p ~/.openclaw/workspace/skills
cp -R skills/dingtalk-form ~/.openclaw/workspace/skills/
```

通过 `dws profile list --format json` 核实机器人所在组织及该组织唯一默认账号，在安装后的 `dingtalk-form` 目录创建 `organization.json`：

```json
{"corpId": "管理员确认的机器人组织corpId"}
```

该文件只保存组织标识，请勿填入凭据或提交到 Git。DWS 授权用于查询目录；机器人私聊采用配对模式时，还需批准发起人的原生配对申请。群投放前应将机器人加入目标群。开始新会话前确认宿主已加载该工作区 Skill；普通使用者无需填写技术 ID。
...
`target.id` 使用原始 ID，不带 `user:`、`group:` 等前缀。ID 必须来自用户明确提供的信息或已核实的钉钉目录，不能根据姓名猜测。`type=user` 只接受该用户填写，不能另外设置 `respondentUserIds`；`type=group` 必须显式列出 1–1000 个不重复的填写人 staffId。机器人必须具有向目标用户或群投放卡片的权限，接口投放失败时工具会返回失败。
...
- 等待时间默认 5 分钟，可用 `timeoutMinutes` 指定 1–4320 分钟（最长 3 天），只适用于定向表单。工具返回 `deadline` 和 `questionId`。截止时回传已收集结果，未回应的人标为 `missing`，整体标为 `expired`；旧卡不能继续提交。
...
发起人在原会话说“查看我正在收集的表单”，机器人调用 `{"action":"list"}`，取得标题、`questionId`、截止时间和已回应人数。说“取消物资需求表单”时，先从列表确定唯一表单，再调用 `{"action":"cancel","questionId":"真实ID"}`；重名时须确认，不能猜 ID。
...
这不是匿名问卷或面向任意群成员的公开表单。即时工具仍不支持从钉钉上下文之外发起投放；已在钉钉中确认并绑定的计划可通过[定时自动收集](scheduled-form-collection.md)入口执行。`questions` 和 `fields` 两种表单定义都可搭配 `target`，无需更换内置模板。
...
标题和整体状态使用顶层列表。单人和多人汇总均将个人状态放在填写人同一行的括号内，例如 `- 填写人：成员甲（已提交）`，不另起“回应状态”一行；个人状态包括已提交、空提交、已取消填写和未回应，收齐、超时及发起人取消均使用这一格式。不同填写人的结果之间空一行，答案使用简单列表；生成的列表行从行首开始、行尾不添加空格，避免强制换行、前导对齐空格、嵌套列表和表格。多行答案可单独使用代码块保留原文，排版规范不删除题目或答案本身的内容、空格及缩进。

中文汇总使用以下格式，收集状态按实际结果使用“已完成”“已超时”或“已取消”；“已完成”不代表每位填写人都提交了答案，个人状态与填写人姓名同一行展示。
...
配合 `dingtalk-form` Skill 和 `dingtalk_form_schedule` 工具，可以使用 OpenClaw 内置 cron 保存确认后的模板并定时执行，执行阶段无需交互引导。名单固定为配置时核实的成员，结果直接回传到原会话；计划跨重启保留，已发出的未完成表单仍在重启时终止。详细设置、管理和异常处理见[定时自动收集](scheduled-form-collection.md)。
