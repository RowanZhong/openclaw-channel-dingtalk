...
- 等待时间默认 5 分钟，可用 `timeoutMinutes` 指定 1–4320 分钟（最长 72 小时），只适用于定向表单。工具返回 `deadline` 和 `questionId`。截止时回传已收集结果，未回应的人标为 `missing`，整体标为 `expired`；旧卡不能继续提交。
...
**网关重启会终止未完成表单。**本阶段不持久化答案，也不在重启后恢复收集。
