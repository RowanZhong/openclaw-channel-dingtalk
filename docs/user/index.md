<script setup>
import formGuideUrl from './dingtalk-form-guide.html?url'
</script>

# 用户文档

这部分面向插件使用者，覆盖从接入到排障的主要路径。

## 推荐阅读顺序

1. [安装](getting-started/install.md)
2. [更新](getting-started/update.md)
3. [配置](getting-started/configure.md)
4. [钉钉权限与凭证](getting-started/permissions.md)
5. 按需查阅下面的功能页与参考页

## 你会在这里找到什么

- 如何安装和更新插件
- 如何配置 OpenClaw 与钉钉应用
- 插件支持哪些消息与回复模式
- 多 Agent、多机器人绑定如何配置
- 常见连接与权限问题如何排查

## 快速开始

- [安装](getting-started/install.md)
- [更新](getting-started/update.md)
- [配置](getting-started/configure.md)
- [钉钉权限与凭证](getting-started/permissions.md)

## 功能说明

- [消息类型支持](features/message-types.md)
- [回复模式](features/reply-modes.md)
- [AI 卡片](features/ai-card.md)
- [表单互动卡片](features/form-interactive-card.md)
- [定时自动收集](features/scheduled-form-collection.md)
- [钉钉文档 API](features/dingtalk-docs-api.md)
- [反馈学习](features/feedback-learning.md)
- [/btw 旁路问答](features/btw.md)
- [多 Agent 与多机器人绑定](features/multi-agent-bindings.md)
- [@多助手路由](features/at-agent-routing.md)

## 互动表单图解

用信息图了解表单设计引导、六种投放场景、定时自动收集和企业实例。HTML 为单文件，下载后可离线打开。

<a :href="formGuideUrl" target="_blank" rel="noopener noreferrer">打开 HTML 使用指南</a> · <a :href="formGuideUrl" download="dingtalk-form-guide.html">下载 HTML 文件</a>

## 参考

- [配置项](reference/configuration.md)
- [安全策略](reference/security-policies.md)
- [API 消耗说明](reference/api-usage-and-cost.md)

## 故障排查

- [故障排查总览](troubleshooting/index.md)
- [连接问题](troubleshooting/connection.md)
- [连接问题详解（中文）](troubleshooting/connection.zh-CN.md)
