...
实现时配置键由 spec 中的 `gateway` 更名为 **`gatewayCapabilities`**（`channels.dingtalk.gatewayCapabilities`）。命名演进与理由：

1. spec 用 `gateway`，但宿主 `OpenClawConfig` 已有顶层 `gateway` 配置（网络/发现/角色策略），插件级同名字段会类型冲突；`tools` 同样被宿主占用（`OpenClawConfig.tools`），因此不能直接采用官方 connector 的 `tools.*` 顶层形态。
2. 初版实现据此取 `gatewayRpc`，语义精确但过于术语化。
3. 复审后改为 `gatewayCapabilities`：保留 `gateway` 前缀以避开宿主 `gateway` / `tools`，用 `capabilities` 提升可读性；不直接用裸 `capabilities`，是因为本插件已存在 `dingtalkPlugin.capabilities`（频道特性元数据：`chatTypes` / `reactions` / `threads` / `media` 等），同名会让配置键与频道能力声明混淆。

`gatewayRpc` 未随任何版本发布，改名无向后兼容成本。其余能力开关形态（`tools.docs` / `tools.proactiveSend`、`docs.allowedSpaceIds`、`send.allowedTargets`）与 spec 一致。

日志前缀 `[DingTalk][GatewayRPC][Denied]` 保持不变：它命名的是被拒绝的**调用面**（Gateway RPC），与配置块名 `gatewayCapabilities` 是两个不同的名词，也与文档页「Gateway RPC 兼容层」一致。

审核后明确的边界（均已写入用户文档）：

- 账号级 `gatewayCapabilities` 与渠道级 **按子键合并**（`tools` / `docs` / `send` 三组分别合并），而不是对象级整体替换：整体替换会让账号级的局部配置静默移除渠道级白名单（fail-open），与"能力开关是安全控制"的定位冲突。
...
- [x] `src/config-schema.ts`：新增 `gatewayCapabilities.tools/docs/send` schema 与一致性校验（`allowedTargets` 项必须以 `user:`/`group:` 开头，白名单至少一项）。
- [x] `src/config.ts`：`resolveGatewayCapabilityConfig` + 多账号按子键合并（`mergeGatewayCapabilitiesConfig`）+ 单测。
- [x] `openclaw.plugin.json`：顶层与账号级 schema 同步 `gatewayCapabilities`（含 `uiHints`），并在 `tests/unit/plugin-manifest.test.ts` 加守护断言。
...
- [ ] 关闭 `gatewayCapabilities.tools.docs` 后，`dingtalk.docs.create` 与 `dingtalk-connector.docs.create` 均返回禁用错误。
- [ ] 配置 `allowedSpaceIds` 后，未列入的 spaceId 请求被拒绝；未配置时行为不变。
- [ ] 配置 `allowedSpaceIds` 后，`docs.append` 与不带 `spaceId` 的 `docs.search` 返回明确拒绝原因。
- [ ] 关闭 `gatewayCapabilities.tools.proactiveSend` 后，`sendToUser/sendToGroup/send` 全部拒绝；聊天回复通道（inbound reply）不受影响。
