...
TIMEOUT_MESSAGE = "请填写 1–1440 的整数分钟数，最长不超过 24 小时；未发送表单，请修改时长。"
...
def duration(request):
    if (not isinstance(request, dict) or set(request) != {"value", "unit"}
            or request["unit"] not in ("minutes", "hours")):
        fail("invalid_request", "时长校验需提供原始 value 和 unit（minutes 或 hours）。")
    value = request["value"]
