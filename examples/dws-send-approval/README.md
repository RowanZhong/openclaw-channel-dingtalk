...
本地 182 项回归和两版真实宿主 SDK/审批兼容测试通过；专用模板已在钉钉开发平台发布，两版均已验证桌面卡片显示、按钮回调和多行输入保存。本轮已改为文本目标校验、可见单选/多选和分组设置页，并增加绝对过期及历史卡停用；两版已完成真实群 @接收、即时审批卡、批准后原文引用回复、话题关联、名称自动补全及原操作卡继续可用的桌面真机验收；手机端及 15000 Pod 压测尚未完成，详见开发文档的验证记录。安装不自动开启监听，也不替代主 Agent 全局权限管理。
